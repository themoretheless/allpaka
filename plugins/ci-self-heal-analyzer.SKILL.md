...
- Объём логов ограничен `--max-log`, потому что полный лог сборки бывает на десятки
  мегабайт и не влезает в разбор. Качаем целиком в файл и обрезаем после: конвейер
  `gh … | head -c` при обрезке теряет всё — gh уходит по SIGPIPE, `pipefail` делает
  конвейер неуспешным, и ветка `||` стирает уже записанный кусок.
- Скрипт живёт под `set -euo pipefail`, поэтому каждый `$(grep … | …)` обязан носить
  `|| true`: grep без совпадения возвращает 1, и присваивание роняет разбор молча —
  пустой stdout и rc 1 вместо отчёта. Это поймал прогон на реальном красном запуске,
  заглушки его не воспроизводили.
